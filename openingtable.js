...
[
  ...
  function buildTable(data){
     var table = document.getElementById('openingTable')
   
      let tempCounter =0
     for (var i = 135; i < data.length; i++){
    if(data[i].pgn.length <= 6) { 
    var row = `<tr>
         <td>${data[i].eco}</td>
         <td>${data[i].name}</td>
         <td>${data[i].pgn}</td>
    ...
    if(tempCounter==8){break}
    }
}


buildTable(openingArray)

let pgnCheck = document.getElementById('pgnList')
const chessPieces = document.querySelectorAll('.chessPiece')
let whiteOrBlackTracker = 1
const matchedOpenings = [] 


chessPieces.forEach(chessPiece => {
  chessPiece.addEventListener("dragend", (event) => 
  {
    pgnUpdate()
  });
...
openingArray.sort((a, b) => a.pgn.length - b.pgn.length);
updateTable(openingArray)
if( matchedOpenings.length == 0){
  return
}else{
  clearTable()
  addMatchedOpenings(matchedOpenings)
...
function updateTable(data){
  ...
  let pgnPointer = data[i].pgn
  let pgnUpdatedString = pgnCheck.innerText.slice(7)
  x = pgnPointer.length - pgnUpdatedString.length
  x = 0-x
  pgnCompare = pgnPointer.slice(0,x)
  
    if(pgnCompare==pgnUpdatedString){
      matchedOpenings.push(data[i])
    }
 }
  return matchedOpenings.length
...
function clearTable(){
  $("#openingTable").find("tbody:gt(0)").remove();
}

function addBlankRows(rowCounterTemp) {
  if(rowCounterTemp < 7){
  var table = document.getElementById('openingTable')
  for(var i=0; i < 7; i++) {
    var rowBlank = `<tr>
...
function addMatchedOpenings(data) {
  var table = document.getElementById('openingTable')
  let x = 0
  for(var i =0; i < data.length; i++){
  var row = `<tr>
  ...
  if(i == 0) {
    ...
    document.getElementById('dbECO').innerText = " ";
    document.getElementById('dbECO').innerHTML = "<br><br><br>ECO<br><br><br><br>";
    document.getElementById('dbECO').innerHTML += data[i].eco; 
